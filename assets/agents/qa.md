...
commits belonging to this ticket. A commit from another ticket means two Builders shared a
working tree and raced; report it as a FAIL with the offending SHA, and check
`git log origin/main..main` for contamination of local `main` too. Use `npm run preview` and code inspection to
