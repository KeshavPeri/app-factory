...
> **AMENDED 16 Aug 2026 — the batch limit is now 3, and nightly.** Both starting settings above were
> deliberately conservative and both have been superseded by observation, which is what this section
> asked for. Nightly runs were adopted on 9 Aug (see `deltas.md` D2 — the `daily` preset avoids a CLI
> step, and the cheap exit in §4.4 makes an empty night nearly free). The batch limit went to 3 on
> 16 Aug after four clean multi-ticket runs in which worktree isolation held and the per-ticket
> decisions files never collided. **The rationale in the two bullets above is unchanged and still
> correct** — batching still beats fragmenting, and the fixed per-session overhead is still the
> reason. Only the number moved.
>
> Two costs came with it, both real: quota, since three tickets is roughly half again as many
> subagent dispatches on one subscription; and merge-order coupling, since three branches all fork
> from the same `main` and any two touching the same file conflict on the second merge regardless of
> whether the tickets depend on each other. The orchestrator prompt (v0.5) now requires both to be
> surfaced in the end-of-run note.
>
> **The binding copy of this number is `fpl-advisor/CLAUDE.md`**, not this document and not the
> routine's saved prompt — the routine holds its own copy that only changes when the prompt is
> re-pasted, so the two can and did drift. On the 16 Aug run the orchestrator found the disagreement,
> followed `CLAUDE.md`, and flagged it. That is the correct resolution and it should stay that way.
